...
**注**:このドキュメントはメンテナンスされておらず、古くなっている可能性があります。

IBCモジュールは次のイベントを発行します。 タイプ `メッセージ`、
`action`の属性キーを使用すると、各メッセージの最初のイベントを表します
SDKのbaseappによって発行されたとおりに処理されます。 各IBCTAOメッセージは
また、モジュール名を「ibc_sub-modulename」の形式で出力します。

チャネルハンドシェイクのすべてのイベント、 `SendPacket`、` RecvPacket`、 `AcknowledgePacket`、
`TimeoutPacket`と` TimeoutOnClose`は、ここで指定されていない追加のイベントを発行します。
IBCアプリケーションへのコールバック。
